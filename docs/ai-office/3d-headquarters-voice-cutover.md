...
## State-truthful briefings (2026-09-25)

Agents now speak by persisted state, never by guess. Active work keeps the existing active briefing; a role whose task finished says "I completed <task> <just now / N minutes ago / earlier>" and states the result from persisted facts only (artifact type and size, files it wrote, its recorded run, a real test result, delivery state) — no model text is copied and no model is called for speech; waiting names the unfinished dependency; blocked uses the existing safe failure summary; a role with no task in the project says it was not assigned and did not take part. Completed work is never worded as active, and a role that starts a new run is ACTIVE again. The greeting signature includes completion so a changed situation is announced while an unchanged one respects the cooldown.

Acceptance used a copy of the completed project `9b70d3a6-5c5d-4d79-aabf-25df5c417a9e` (no runner, nothing executed; the original database is untouched): Boss walked to the Orchestrator, PO, Architect, UX, QA, Code Review, Developer and Release, plus Security (idle, not participating). Every spoken subtitle equalled the deterministic briefing built from that project's persisted state, each with a local native voice start; the detailed panel matched too. Walking away cancelled speech, Voice OFF produced no utterances, at most one utterance played at a time, and no external host was contacted. Security was greeted in passing on the way to QA and, per the 90 s greeting cooldown, stayed silent when Boss reached it; its exact text was spoken then and its detailed panel was verified.

## Production cutover (2026-09-25)

- `/office` is the 3D Headquarters; the previous dashboard-style Office is preserved unchanged at `/office/classic` (same protected layout, same `getOfficeDb()` state layer, same actions — no second runner or state system).
- `/office/headquarters` redirects to `/office` (query `project` preserved). `/office/headquarters-state` and `/office/world-prototype` are unchanged.
- Sidebar: "Headquarters" (`/office`) and a small "Classic Office" entry. Every 3D fallback (WebGL unavailable, mobile/coarse pointer, render-error boundary, loading state) links to `/office/classic`, never back to the 3D route.
- Mutating server actions revalidate the `/office` layout so both experiences refresh.
- The fullscreen scene covers the shell sidebar by design; Sign out is reached from Classic Office or any other Office page.
